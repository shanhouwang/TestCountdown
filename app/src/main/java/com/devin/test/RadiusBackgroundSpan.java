...
public class RadiusBackgroundSpan extends ReplacementSpan {
     ...
     * @param padding  内间距
     */
    public RadiusBackgroundSpan(int bgColor, int txtColor, int radius, int padding) {
    ...
    public void draw(Canvas canvas, CharSequence text, int start, int end, float x, int top, int y, int bottom, Paint paint) {
        ...
        RectF oval = new RectF(x - mPadding, y + paint.ascent() - mPadding, x + mSize + mPadding, y + paint.descent() + mPadding);
        // 设置文字背景矩形，x为span其实左上角相对整个TextView的x值，y为span左上角相对整个View的y值。paint.ascent()获得文字上边缘，paint.descent()获得文字下边缘
        canvas.drawRoundRect(oval, mRadius, mRadius, paint);//绘制圆角矩形，第二个参数是x半径，第三个参数是y半径
        paint.setColor(mTxtColor == 0 ? paint.getColor() : mTxtColor);// 恢复画笔的文字颜色
